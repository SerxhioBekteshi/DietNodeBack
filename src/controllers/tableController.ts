...
const getAll = <T>(TableClass: TableConstructor<T>) => async (req: any, res: any, next: any) => {
  const table = new TableClass(req.body, req.user);
  const data = await table.initialize();
  res.json(data);
};
const selectAll = <T>(TableClass: TableConstructor<T>) => async (req: any, res: any, next: any) => {
  const table = new TableClass(req.body, req.user);
  const data = await table.selectAll();
  res.json(data);
};
const deleteSelected = <T>(TableClass: TableConstructor<T>) => async (req: any, res: any, next: any) => {
  const table = new TableClass(req.body, req.user);
